...
every 30 s and terminates connections that did not answer the previous ping.
On the same timer it re-checks every device token against the database: a
deleted or deactivated token is disconnected with close code `4000`.
Revoking a token in the dashboard disconnects it immediately (close frame,
then a hard terminate after 2 s if the device does not complete the closing
handshake).
...
| `tcp-data` | `connId`, `data` (base64 string) | **Legacy v1** data message. Accepted from any peer. |
...
| `tcp-open` | `connId` (UUID), `tunnelId`, `localPort` | A public client connected (TCP tunnel) or an HTTP request arrived (HTTP tunnel): open a TCP connection to `localhost:<localPort>`. Devices must only accept ports of their own configured tunnels (and, when `tunnelId` is present, only the tunnel registered for that port); otherwise reply `tcp-close`. |
