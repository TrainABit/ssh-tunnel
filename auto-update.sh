#!/bin/bash
# ================================================================
# TunnelVault Server Auto-Updater (signed GitHub releases, no git)
#
# Installed by `install-server.sh --auto-update --release-pubkey FILE` as
# /opt/tunnelvault/auto-update.sh (root-owned) and run as root by
# tunnelvault-autoupdate.timer. Manual run:
#   sudo /opt/tunnelvault/auto-update.sh [--dry-run]
#
...
# `install-server.sh --upgrade --yes` from the verified tree (the release ships
# a prebuilt frontend/dist). Any failure aborts without touching the
# installation (fail closed). Log: journal + /var/log/tunnelvault-update.log.
#
...
TV_ROLE="server"
TV_INSTALL_DIR="${TUNNELVAULT_INSTALL_DIR:-/opt/tunnelvault}"
TV_INSTALLER="install-server.sh"
TV_INSTALLER_ARGS=(--upgrade --yes)
TV_LOG_FILE="${TUNNELVAULT_UPDATE_LOG-/var/log/tunnelvault-update.log}"
TV_LOCK_FILE="${TUNNELVAULT_UPDATE_LOCK:-/run/tunnelvault-update.lock}"
